...
*/
        ...
        $rootScope.coords = {"lat": 33.702, "lon": -118.002};
navigator.geolocation.getCurrentPosition(function(pos) {
        }, function(err) {
          alert('Unable to get location: ' + err.message);
        });
...
if ($rootScope.geoD < 1) { $rootScope.geoD = Math.round($rootScope.geoD * 1000); }
  ...
  $rootScope.newMatch = function () {  
    dbSvc.ngrUtil.save( $rootScope.vGM00, 
      function(newRec, respHeaders) {  
        $rootScope.matchID = newRec._id;     console.log($rootScope.matchID);  }
    );
/* 
...
.controller('MainCtrl', function($rootScope, $scope, $timeout, $http, dbSvc) {
...
$scope.timer = function() {
  ...
  */
  // $rootScope.appLog += 'y';
  $timeout($scope.timer, 1100);    // 1100 milli-second delay
};
$scope.timer();
...
//  5a6b9e9da07bee72000109a7   5ae78ec6150b711200002e1a  
//  return $resource('https://api.airtable.com/v0/app0hohtq4b1nM0Kb/Players?api_key=key66fQg5IghIIQmb');
//  var url = 'https://gwfl-256d.restdb.io/rest/utility?apikey=5821f61550e9b39131fe1b6f';    
//  var url = 'https://api.airtable.com/v0/app0hohtq4b1nM0Kb/Players?api_key=key66fQg5IghIIQmb';
